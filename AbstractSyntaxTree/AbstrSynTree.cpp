...
std::string ASTNode::show(bool details)
{
  ...
  if(name_ != "none")
    temp << name_ << ", ";
  if(type_ == "namespace" || type_ == "class" || type_ == "interface" || type_ == "struct" || type_ == "function" || type_ == "typedef")
    temp << package_ << ", ";
  if (details)
  {
...
void AbstrSynTree::add(ASTNode* pNode)
{
  ...
  if ( pNode->type_ == "class" || pNode->type_ == "struct" ||pNode->type_ == "enum" || pNode->type_ == "interface" || pNode->type_ == "alias" )
